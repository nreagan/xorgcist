...
def parse_xinput(text):
    ...
    for line in text.splitlines():
        ...
        if m and "XTEST" not in m.group(1) and m.group(1) not in names:
            names.append(m.group(1))
    return sorted(names, key=lambda n: "touch" not in n.lower())
...
def emit_touch_script(state):
    ...
    rows = [t for t in state["touch"] if t["device"].strip()]
    ...
    for t in rows:
        ...
        lines.append('xinput set-prop %s --type=float "Coordinate Transformation Matrix" %s' % (
            shlex.quote("pointer:" + name),
            " ".join(fmt_num(v) for v in touch_matrix(shown, d))))
    return "\n".join(lines) + "\n"
...
def build_touch_rows(state, ui):
    ...
    for i, t in enumerate(state["touch"]):
        dev = ttk.Combobox(frame, values=state["devices"], width=22)
        dev.set(t["device"])
